...
if create_auditor_migrations_module:
    ...
    if not os.path.exists(
        os.path.join(settings.BASE_DIR, "auditor_migrations", "__init__.py")
    ):
        open(
            os.path.join(settings.BASE_DIR, "auditor_migrations", "__init__.py"), "a"
        ).close()
        ...
        if app.models != None:
            ...
            for model in initial_models:
                if not model.startswith("audit"):
                    ...
                    audits_models["Audit" + model] = type(
                        "Audit" + model,
                        (models.Model,),
                        {
                    ...
                    def post_save(sender, instance, created, **kwargs):
                        ...
                        for field in instance._meta.get_fields():
                            if not field.is_relation:
                                model_metadata[field.name] = str(
                                    getattr(instance, field.name)
                                )
                        model_obj.objects.create(
                            id_model=instance.id,
                            user_metadata=json.dumps(
                                MetadataManager.get_user_metadata(), skipkeys=True
                            ),
                            request_metadata=json.dumps(
                                MetadataManager.get_request_metadata(), skipkeys=True
                            ),
                    ...
                    def post_delete(sender, instance, **kwargs):
                        ...
                        model_obj.objects.create(
                            id_model=instance.id,
                            user_metadata=json.dumps(
                                MetadataManager.get_user_metadata()
                            ),
                            request_metadata=json.dumps(
                                MetadataManager.get_request_metadata()
                            ),
                            model_metadata=model_metadata,
                            action="delete",
                        )
                    ...
                    models.signals.post_save.connect(
                        post_save,
                        sender=f"{app.label}.{model}",
                        weak=False,
                        dispatch_uid=f"{app.label}.{model}.post_save",
                    )
                    models.signals.post_delete.connect(
                        post_delete,
                        sender=f"{app.label}.{model}",
                        weak=False,
                        dispatch_uid=f"{app.label}.{model}.post_delete",
                    )
